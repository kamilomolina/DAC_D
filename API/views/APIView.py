...
FORMATO_DEFAULT = 1
TELEGRAM_BOT_URL = "https://api.telegram.org/bot6000801458:AAHsVp6MyKBu8ak_PCkHw3rbmfM_vEsyAWc/sendMessage"
AGRUPACION_PROVEEDORES = 1


class SetInsertUpdateProveedor(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if auth_header:
            if auth_header != TOKEN:
                return Response(
                    {"error": "Token no válido o formato de autorización incorrecto."},
        ...
        def safe(val):
            return val if val not in [None, "", "null", "NULL"] else None

        # === DATOS MÍNIMOS COMUNES ===
        Codigo = safe(request.POST.get("Codigo"))
        NombreConocido = safe(request.POST.get("NombreConocido"))
        Contacto = safe(request.POST.get("Contacto"))
        fkCiudad = safe(request.POST.get("fkCiudad"))
        Direccion = safe(request.POST.get("Direccion"))
        Telefonos = safe(request.POST.get("Telefonos"))
        Fax = safe(request.POST.get("Fax"))
        RTN = safe(request.POST.get("RTN"))
        Correo = safe(request.POST.get("Correo"))
        TerminosPago = safe(request.POST.get("TerminosPago"))  # Días de crédito
        userName = safe(request.POST.get("userName"))

        try:
            # === QUERIES SEGÚN BASE DE DATOS ===
            queries = {
                # master_control, super_dc, super_carbajal → misma estructura
                "master_control": """
                    INSERT INTO proveedores (
                        Codigo, NombreConocido, NombreComercial, Contacto, fkCiudad, Direccion,
                        Telefonos, Fax, RTN, Correo, LimiteCredito, TerminosPago, Descuento,
                        fkEstadoCuenta, fkEstadoCliente, Notas, Clase, Tipo, AddUser, Created
                    ) VALUES (
                        %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,0,IFNULL(%s,0),0,1,5,NULL,2,0,%s,NOW()
                    )
                    ON DUPLICATE KEY UPDATE
                        NombreConocido = VALUES(NombreConocido),
                        NombreComercial = VALUES(NombreComercial),
                        Contacto = VALUES(Contacto),
                        fkCiudad = VALUES(fkCiudad),
                        Direccion = VALUES(Direccion),
                        Telefonos = VALUES(Telefonos),
                        Fax = VALUES(Fax),
                        RTN = VALUES(RTN),
                        Correo = VALUES(Correo),
                        TerminosPago = VALUES(TerminosPago),
                        EditUser = VALUES(AddUser),
                        LastUpdated = NOW();
                """,
                # mca_data.mant_proveedores
                "mca_data": """
                    INSERT INTO mant_proveedores (
                        CodigoProveedor, NombreProveedor, Contacto, fkCiudad, Direccion,
                        Telefono, Fax, RTN, CorreoElectronico, DiasCredito, UsuarioCreo, Creado
                    ) VALUES (
                        %s,%s,%s,%s,%s,%s,%s,%s,%s,IFNULL(%s,0),%s,NOW()
                    )
                    ON DUPLICATE KEY UPDATE
                        ...
                        DiasCredito = VALUES(DiasCredito),
                        UsuarioEdito = VALUES(UsuarioCreo),
                        Editado = NOW();
                """,
                # bancario.cat_proveedores
                "bancario": """
                    INSERT INTO cat_proveedores (
                        Id_Proveedor, Nombre_Proveedor, Dias_Credito, Id_Acreedor
                    ) VALUES (%s,%s,IFNULL(%s,0),0)
                    ON DUPLICATE KEY UPDATE
            ...
            for conn in conexiones:
                ...
                with db.cursor() as cursor:
                    if db_name in ["master_control", "super_dc", "super_carbajal"]:
                        params = (
                            Codigo,
                            NombreConocido,
                            NombreConocido,
                        ...
                        cursor.execute(queries["master_control"], params)

                    elif db_name == "mca_data":
                        params = (
                            Codigo,
                            NombreConocido,
                            Contacto,
                        ...
                        cursor.execute(queries["mca_data"], params)

                    elif db_name == "bancario":
                        params = (Codigo, NombreConocido, TerminosPago)
                        cursor.execute(queries["bancario"], params)
...
class SetPrincipalArticulosProveedorSupermercado(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            with createdConn.cursor() as cursor:
...
class loginRequestV2(APIView):
    def post(self, request, *args, **kwargs):
        ...
        result = subprocess.run(
            ["php", php_script_path, password], stdout=subprocess.PIPE
        )
        ...
        try:
            with connections["global_nube"].cursor() as cursor:
                cursor.callproc("SDK_GET_USER_ACCESS", [username, password64])
...
class obtenerEmpleadosDeptos(APIView):
    def get(self, request, *args, **kwargs):
        try:
            with connections["universal"].cursor() as cursor:
                cursor.callproc("TH_GET_EMPLEADOS_DEPTO")
                empleadosQuery = cursor.fetchall()

                # Fetch column names
                columns = [col[0] for col in cursor.description]

                # Create a list of dictionaries mapping column names to values
                empleados_list = [dict(zip(columns, row)) for row in empleadosQuery]

            return JsonResponse({"save": 1, "data": empleados_list}, safe=False)
        except Exception as e:
...
class ReporteRankingProveedores(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            with defaultConn.cursor() as cursor:
                cursor.callproc(
                    stored_procedure, [almacen, proveedor, producto, date1, date2]
                )
                column_names = [desc[0] for desc in cursor.description]
                rankingProveedoresData = [
                    dict(zip(map(str, column_names), row)) for row in cursor.fetchall()
                ]
...
class ReporteRankingProveedoresDetalles(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            with defaultConn.cursor() as cursor:
                cursor.callproc(
                    "DAC_REPORTE_RANKING_PROVEEDORES_DETAILS_v2",
                    [almacen, proveedor, producto, date1, date2],
                )
                column_names = [desc[0] for desc in cursor.description]
                rankingProveedoresDetallesData = [
                    dict(zip(map(str, column_names), row)) for row in cursor.fetchall()
                ]
...
class updateSaldosMovil(APIView):
    def get(self, request, *args, **kwargs):
        ...
        engine_source = create_engine(
            "mysql+pymysql://root:{}@190.4.11.58/super_carbajal".format(password)
        )
        ...
        try:
            articulos_bodega = pd.read_sql(
                "CALL API_UPDATE_SALDOS_MOVIL()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            presentaciones = pd.read_sql(
                "CALL SZ_GET_PRESENTACIONES_ALL()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            articulos_escalas = pd.read_sql(
                "CALL SZ_GET_ARTICULOS_ESCALA_ALL()", con=engine_source
            )
        except Exception as e:
        ...
        engine_destination = create_engine(
            "mysql+pymysql://root:{}@190.4.11.58/pedidos_posca".format(password2)
        )
        ...
        with engine_destination.connect() as conn:
            ...
            if articulos_bodega is not None:
                try:
                    ...
                    print(
                        "Error al insertar datos Articulos Bodega Super DC Nube",
                        format(str(e)),
                    )
                    error_info = {
            ...
            if clientes is not None:
                try:
                    ...
                    print(
                        "Error al insertar datos Clientes Super DC Nube", format(str(e))
                    )
                    error_info = {
            ...
            if presentaciones is not None:
                try:
                    ...
                    print(
                        "Error al insertar datos Presentaciones Super DC Nube",
                        format(str(e)),
                    )
                    error_info = {
            ...
            if grupos is not None:
                try:
                    ...
                    print(
                        "Error al insertar datos Grupos Super DC Nube", format(str(e))
                    )
                    error_info = {
            ...
            if subgrupos is not None:
                try:
                    ...
                    print(
                        "Error al insertar datos Subgrupos Super DC Nube",
                        format(str(e)),
                    )
                    error_info = {
            ...
            if medidas is not None:
                try:
                    ...
                    print(
                        "Error al insertar datos Medidas Super DC Nube", format(str(e))
                    )
                    error_info = {
            ...
            if articulos_escalas is not None:
                try:
                    ...
                    print(
                        "Error al insertar datos Articulos Escala Super DC Nube",
                        format(str(e)),
                    )
                    error_info = {
            ...
            if articulos is not None:
                try:
                    ...
                    print(
                        "Error al insertar datos Articulos Super DC Nube",
                        format(str(e)),
                    )
                    error_info = {
        ...
        return JsonResponse(
            {"success": True, "time": formatted_elapsed_time, "error": len(error_list)}
        )
...
class UpdateCAINumeroIngresado(APIView):
    def get(self, request, maquina, *args, **kwargs):
        ...
        try:
            config_facturacion = pd.read_sql(
                "CALL SZ_GET_CONFIG_FACTURACION_ALL()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            facturas_ingresadas = pd.read_sql(
                "CALL SZ_GET_FACTURAS_INGRESADAS_CAI()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            nc_ingresadas = pd.read_sql(
                "CALL SZ_GET_NOTAS_CREDITO_INGRESADAS_CAI()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            nd_ingresadas = pd.read_sql(
                "CALL SZ_GET_NOTAS_DEBITO_INGRESADAS_CAI()", con=engine_source
            )
        except Exception as e:
                        ...
                        try:
                            ...
                            try:
                                result = session.execute(
                                    "CALL SZ_GET_STATUS_MAQUINA_LOCAL(:param)",
                                    {"param": PKconfig},
                                )
                                local = (
                                    result.fetchone()[0]
                                ...
                                if local == 1:
                                    local_mode = "LOCAL"
                                    descripcion_text = (
                                        "DEBE SUBIR DATOS Y CAMBIAR A MODO SERVIDOR"
                                    )
                                else:
                                    local_mode = "SERVIDOR"
                            ...
                            if local == 0:
                                with engine_destination.connect() as conn:
                                    ...
                                    try:
                                        conn.execute(
                                            "TRUNCATE TABLE configuracion_facturacion;"
                                        )
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Config Facturacion ",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                if not facturas_ingresadas.empty:
                                    for factura in facturas_ingresadas.itertuples():
                                        ...
                                        if pd.isna(factura.fecha_ingreso):
                                            ...
                                            fecha_ingreso = (
                                                factura.fecha_ingreso.to_pydatetime()
                                            )
                                        ...
                                        try:
                                            ...
                                            session.rollback()
                                            print(
                                                "Error durante la inserción de facturas:",
                                                str(e),
                                            )
                                        except Exception as e:
                                            session.rollback()
                                ...
                                if not nc_ingresadas.empty:
                                    for nc in nc_ingresadas.itertuples():
                                        ...
                                        try:
                                            ...
                                            session.rollback()
                                            print(
                                                "Error durante la inserción de notas credito:",
                                                str(e),
                                            )
                                        except Exception as e:
                                            session.rollback()
                                ...
                                if not nd_ingresadas.empty:
                                    for nd in nd_ingresadas.itertuples():
                                        ...
                                        try:
                                            ...
                                            session.rollback()
                                            print(
                                                "Error durante la inserción de notas debito:",
                                                str(e),
                                            )
                                        except Exception as e:
                                            session.rollback()
        ...
        for error in error_list:
            ...
            try:
                ...
                with gsLocal.cursor() as cursor:
                    cursor.callproc(
                        "GS_INSERT_LOG_MIGRACION_DATOS_SUPER",
                        [asunto, descripcion, maquina],
                    )
...
class UpdateDataSuperToLocal(APIView):
    def get(self, request, maquina, *args, **kwargs):
        ...
        password = "$Admincarba24*_"
        engine_source = create_engine(
            "mysql+pymysql://root:{}@54.81.57.249/master_control".format(password)
        )

        try:
            presentaciones_niveles = pd.read_sql(
                "CALL SZ_GET_PRESENTACIONES_NIVELES()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            autorizacion_modo_local = pd.read_sql(
                "CALL SZ_GET_AUTORIZACIONES_LOCALES_ALL()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            cxc_notasdebito = pd.read_sql(
                "CALL SZ_GET_CXC_NOTAS_DEBITO_ALL()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            cxc_notascredito = pd.read_sql(
                "CALL SZ_GET_CXC_NOTAS_CREDITO_ALL()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            config_basculas = pd.read_sql(
                "CALL SZ_GET_CONF_BASCULA_ALL()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            config_cantidad_precios = pd.read_sql(
                "CALL SZ_GET_CONFIG_CANTIDAD_PRECIOS_ALL()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            config_impresoras = pd.read_sql(
                "CALL SZ_GET_CONFIG_IMPRESORAS_ALL()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            config_maquinas = pd.read_sql(
                "CALL SZ_GET_CONFIG_MAQUINAS()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            cobros_detalle = pd.read_sql(
                "CALL SZ_GET_COBROS_DETALLE()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            cobros_formas_pago = pd.read_sql(
                "CALL SZ_GET_COBROS_FORMAS_PAGO_ALL()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            tablasregistros = pd.read_sql(
                "CALL SZ_GET_FIRMAS_AUTORIZADAS_ALL()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            rutas_lugares = pd.read_sql(
                "CALL SZ_GET_RUTAS_LUGARES()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            caja_turnos = pd.read_sql(
                "CALL SZ_GET_CAJAS_TURNOS_ALL()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            caja_movimientos = pd.read_sql(
                "CALL SZ_GET_CAJAS_MOVIMIENTOS_DAY()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            control_disponibilidad = pd.read_sql(
                "CALL SZ_GET_CONTROL_DISPONIBILIDAD()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            catalogo_cuentas = pd.read_sql(
                "CALL SZ_GET_CATALOGO_CUENTAS()", con=engine_source
            )
        except Exception as e:
            print("Error Control Disponibilidad Data ", format(str(e)))
            error_info = {
                        ...
                        try:
                            ...
                            if local == 0:
                                with engine_destination.connect() as conn:
                                    ...
                                    try:
                                        tables = [
                                            ...
                                            "configuracion_cantidad_precios",
                                            "configuracion_facturacion",
                                            "configuracion_impresoras",
                                            "configuracion_maquinas",
                                            "cobros",
                                            "cobrosdetalle",
                                            "cobrosformaspago",
                                            "medidas",
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos Presentaciones Niveles Modo Local",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos Catalogo Cuentas Modo Local",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos Control Disponibilidad Modo Local",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos Cajas Movimientos Modo Local",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos Cajas Turnos Modo Local",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Notas Modo Local",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Rutas Lugares Modo Local",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Rutas Zonas Modo Local",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Tablas Registros Modo Local",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    autorizacion_modo_local.to_sql(
                                        "autorizacion_modo_local",
                                        engine_destination,
                                        index=False,
                                        if_exists="append",
                                    ...
                                    print(
                                        "Error al insertar datos en Autorizaciones Modo Local",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Canales",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Articulos",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Presentaciones ",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Clientes ",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en CxC Notas Credito ",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en CxC Notas Debito ",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Medidas",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Subgrupos ",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Grupos ",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Bodegas: {}".format(
                                            str(e)
                                        )
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Empresas: {}".format(
                                            str(e)
                                        )
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Sucursales ",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en CxC ",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Config Bascula ",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    config_cantidad_precios.to_sql(
                                        "configuracion_cantidad_precios",
                                        engine_destination,
                                        index=False,
                                        if_exists="append",
                                    ...
                                    print(
                                        "Error al insertar datos en Config Cantidad Precios ",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Config Impresoras ",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Config Maquinas ",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Cobros ",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Cobros Detalle ",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    ...
                                    print(
                                        "Error al insertar datos en Cobros Formas Pago ",
                                        format(str(e)),
                                    )
                                    error_info = {
                                ...
                                try:
                                    sql = text(
                                        "CALL SZ_UPDATE_CONFIGURACION_BODEGA(:bodega)"
                                    )
                                    ...
                                    print(
                                        "Error en Update Configuracion Bodega ", maquina
                                    )
                                    error_info = {
                                ...
                                try:
                                    sql = text(
                                        "CALL mySP_Update_reiniciar_bodega(:bodega, :hacer, :estado, :todos, :user)"
                                    )
                                    session.execute(
                                        sql,
                                        {
        ...
        for ingreso in ingresosData:
            ...
            try:
                ...
                with bankConn.cursor() as cursor:
                    cursor.callproc(
                        "SP_Insert_Ingresos_Pendientes_pm",
                        [
                            id_factura_sistemas,
                            numero_factura,
                            id_orden_de_pedido,
            ...
            if save == 1:
                try:
                    ...
                    with superNubeConn.cursor() as cursor:
                        cursor.callproc(
                            "API_UPDATE_INGRESOS_PENDIENTES_TRANSFERIDO", [id_factura]
                        )
...
class UpdateDataGlobalToLocal(APIView):
    def get(self, request, maquina, *args, **kwargs):
        ...
        password = "$Carba_admin*20"
        engine_source = create_engine(
            "mysql+pymysql://root:{}@190.4.11.58/global_security".format(password)
        )
        ...
        try:
            usuarios_grupo = pd.read_sql(
                "CALL SZ_GET_USUARIOS_GRUPOS_SUPER_ALL()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            menus_grupo = pd.read_sql(
                "CALL SZ_GET_MENUS_GRUPO_SUPER_ALL()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            usuarios_modulo = pd.read_sql(
                "CALL SZ_GET_USUARIOS_MODULOS_SUPER_ALL()", con=engine_source
            )
        except Exception as e:
        ...
        try:
            usuarios_sucursales = pd.read_sql(
                "CALL SZ_GET_USUARIOS_SUCURSALES_ALL()", con=engine_source
            )
        except Exception as e:
                    ...
                    for db in databases:
                        try:
                            ...
                            with engine_destination.connect() as conn:
                                transaction = (
                                    conn.begin()
                                )  # Inicia una transacción si tus consultas deben ser atómicas
                                try:
                            ...
                            try:
                                ...
                                print(
                                    "Error al insertar datos en Usuarios Admin IT",
                                    format(str(e)),
                                )
                                error_info = {
                            ...
                            try:
                                ...
                                print(
                                    "Error al insertar datos en Usuarios Grupos",
                                    format(str(e)),
                                )
                                error_info = {
                            ...
                            try:
                                ...
                                print(
                                    "Error al insertar datos en Grupos", format(str(e))
                                )
                                error_info = {
                                    "asunto": "Error al insertar datos en Grupos",
                                    "descripcion": str(e),
                            ...
                            try:
                                ...
                                print(
                                    "Error al insertar datos en Menus", format(str(e))
                                )
                                error_info = {
                            ...
                            try:
                                ...
                                print(
                                    "Error al insertar datos en Menus Grupos",
                                    format(str(e)),
                                )
                                error_info = {
                            ...
                            try:
                                ...
                                print(
                                    "Error al insertar datos en Usuarios",
                                    format(str(e)),
                                )
                                error_info = {
                            ...
                            try:
                                ...
                                print(
                                    "Error al insertar datos en Usuarios Modulo",
                                    format(str(e)),
                                )
                                error_info = {
                            ...
                            try:
                                ...
                                print(
                                    "Error al insertar datos en Usuarios Sucursales",
                                    format(str(e)),
                                )
                                error_info = {
    ...
    def get(self, request, *args, **kwargs):
                    ...
                    for db in databases:
                        try:
                            ...
                            try:
                                ...
                                success_text = (
                                    "Conexion a Maquina {} en Modo {}".format(
                                        maquina, local_mode
                                    )
                                )
                                json_info = {
                                ...
                                print("Error en Conexion de Maquina ", maquina)
                                error_text = "Error en Conexion de Maquina  {}".format(
                                    maquina
                                )
                                json_info = {
                                    "asunto": error_text,
        ...
        for json in json_list:
            ...
            try:
                ...
                with superNube.cursor() as cursor:
                    cursor.callproc(
                        "SZ_INSERT_MODO_CONEXION_MAQUINA_LOG",
                        [asunto, maquina, descripcion, modo],
                    )
        ...
        return JsonResponse(
            {"success": True, "time": formatted_elapsed_time, "json": len(json_list)}
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.content_type == "application/json":
            ...
            data = {
                ...
                "assignedDetalle": json.loads(
                    request.POST.get("assignedDetalle", "[]")
        ...
        try:
            with connections["universal"].cursor() as cursor:
                cursor.callproc(
                    "CTRL_INSERT_TOKEN_HEADER_SUCURSAL",
                    [sucursal, num_documento, id_tipo_token, id_modulo, usuario],
                )
                ...
                if existe == 0 or existe == "0":
                    with connections["universal"].cursor() as cursor:
                        cursor.callproc(
                            "CTRL_SOLICITUD_CORREOS", [id_tipo_token, solicitud_id]
                        )
                        ...
                        if emailsQuery:
                            for emails in emailsQuery:
                                ...
                                try:
                                    send_mail(
                                        "SOLICITUD DE TOKEN DE {} CREADA".format(
                                            tipo_token
                                        ),
                                        "",
                                        ...
                                        html_message=render_to_string(
                                            ...
                                            {
                                                "title": "SOLICITUD DE TOKEN DE {} CREADA".format(
                                                    tipo_token
                                                ),
                                                "message": "Buen día, por favor verifica el apartado de Autorizaciones en DAC para visualizar la siguiente solicitud {}".format(
                                                    message
                                                ),
                                                "token": "",
                                                "login_url": login_url,
...
class TokenCancelarSolicitud(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if request.content_type == "application/json":
            ...
            data = {
                ...
                "comentario": request.POST.get(
                    "comentario", "Por politicas de Validacion y Seguridad del Sistema."
                ),
            }
        ...
        try:
            with connections["universal"].cursor() as cursor:
                cursor.callproc(
                    "CTRL_SOLICITUD_CANCELAR_SUCURSAL",
                    [
                        sucursal,
                        num_documento,
                        id_tipo_token,
                            ...
                            send_mail(
                                ...
                                html_message=render_to_string(
                                    "email_template_v1.html",
                                    {
                                        ...
                                        "message": "Buen dia, tu solicitud de token ha sido cancelada, realizaste cambios y ya no aplica la solicitud anterior. MOTIVO: {}".format(
                                            comentario
                                        ),
                                        "token": "",
                                        "login_url": login_url,
...
class TokenGetMisSolicitudes(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            with defaultConn.cursor() as cursor:
                cursor.callproc(
                    "CTRL_GET_MIS_SOLICITUDES_TOKEN", [userName, date1, date2]
                )
                column_names = [desc[0] for desc in cursor.description]
                solicitudesData = [
                    dict(zip(map(str, column_names), row)) for row in cursor.fetchall()
...
class TokenGetSolicitudes(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            with defaultConn.cursor() as cursor:
                cursor.callproc(
                    "CTRL_GET_SOLICITUDES_TOKEN", [user_id, opcion, date1, date2]
                )
                column_names = [desc[0] for desc in cursor.description]
                solicitudesData = [
                    dict(zip(map(str, column_names), row)) for row in cursor.fetchall()
...
class TokenRevisarDetalleSolicitud(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            with defaultConn.cursor() as cursor:
                cursor.callproc(
                    "CTRL_VALIDAR_DETALLES_SOLICITUD_TOKEN", [detalle_id, userName]
                )
...
class TokenSolicitudComentario(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            with defaultConn.cursor() as cursor:
                cursor.callproc(
                    "CTRL_SOLICITUD_TOKEN_COMENTAR_O_RECHAZAR",
                    [detalle_id, comentario, opcion, userName],
                )
...
class TokenValidar_v2(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            with defaultConn.cursor() as cursor:
                cursor.callproc(
                    "CTRL_VALIDAR_TOKEN_v2",
                    [usuario, id_tipo_token, id_modulo, num_documento],
                )
                tokenQuery = cursor.fetchall()
...
class PartidasRecibirDatos(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return JsonResponse({"save": 0, "error": str(e), "resultados": []})

        try:
            with connections["universal"].cursor() as cursor:
                cursor.callproc(
                    "CONTA_PROCESAR_PARTIDAS_BLOQUE_JSON_V2",
                    [json.dumps(data), 200],  # o 500
                )
                row = cursor.fetchone()
                while cursor.nextset():
                    pass
...
def _parse_lista_kpis(kpis):
    """
    "111,132" -> [111,132]
    """
    try:
        return [int(x) for x in str(kpis).split(",") if x.strip().isdigit()]
    except:
        return []


# ======================================================
# CONFIGURACIÓN DE KPIs DESDE BD
# ======================================================


def obtener_config_kpis(lista_kpis_text=None):
    """
    Lee la configuración de KPIs usando SP.
    Si lista_kpis_text es None -> trae todos.
    """

    with connections["estratega_EC2"].cursor() as cursor:
        cursor.callproc("KPI_GET_CONFIG_LIST", [lista_kpis_text])
...
# ======================================================
# EJECUCIÓN DE SP POR COLUMNAS (1 FILA MUCHAS COLUMNAS)
# ======================================================


def obtener_resultado_columnas(conn_name, sp_name, params):
...
# ======================================================
# SP POR REGISTROS (MUCHAS FILAS)
# ======================================================


def ejecutar_kpi_registros(
    conn_name,
    sp_name,
    params,
    columna_kpi="codigo_kpi",
    columna_valor="valor",
    columna_anio="anio",
    columna_mes="mes",
):
    ...
    with conn.cursor() as cursor:
        ...
        idx_kpi = columns.index(columna_kpi)
        idx_val = columns.index(columna_valor)
        idx_anio = columns.index(columna_anio)
        idx_mes = columns.index(columna_mes)

        for row in rows:
            kpis.append(
                {
                    "codigo_kpi": int(row[idx_kpi]) if row[idx_kpi] else 0,
                    "anio": int(row[idx_anio]) if row[idx_anio] else 0,
                    "mes": int(row[idx_mes]) if row[idx_mes] else 0,
                    "resultado": _safe_float(row[idx_val], 0),
                    "comentario": "Generado desde SP por registros",
                }
            )
...
# ======================================================
# EXTRAER KPI DE OBJETO (COLUMNAS)
# ======================================================


def extraer_kpi_de_objeto(objeto, cfg, anio, mes):

    # 👇 TU TABLA USA: nombre_columna
    valor = objeto.get(cfg["nombre_columna"], 0)

    return {
        "codigo_kpi": int(cfg["codigo_kpi"]),
        "anio": int(anio),
        "mes": int(mes),
        "resultado": _safe_float(valor, 0),
...
def construir_parametros(cfg, mes, anio, fecha):
    raw = cfg.get("sp_params", "")
...
def guardar_kpis_destino(lista_kpis, conn_name="estratega_EC2"):
    ...
    with transaction.atomic(using=conn_name):
        with conn.cursor() as cursor:
            for kpi in lista_kpis:

                try:
                    cursor.callproc(
                        "SP_KPI_RESULTADO_UPSERT",
                        [
                            kpi.get("codigo_kpi", 0),
                            kpi.get("anio", 0),
                            kpi.get("mes", 0),
                            kpi.get("resultado", 0),
                            kpi.get("comentario", ""),
                    ...
                    # NO PARAR TODO
                    print("Error guardando KPI:", kpi, str(e))

    return len(lista_kpis)
...
class IndicadoresBalanceSaveData(APIView):

    def get(self, request, kpis=None, *args, **kwargs):
        ...
        try:
            # ================================
            # 1) Parsear KPIs solicitados
            # ================================
            lista_kpis_text = None

            if kpis:
                lista = _parse_lista_kpis(kpis)

                if lista:
                    lista_kpis_text = ",".join(str(x) for x in lista)

            # ================================
            # 2) Leer configuración
            # ================================
            configs = obtener_config_kpis(lista_kpis_text)
            ...
            for cfg in configs:
                ...
                try:
                    ...
                    conn = cfg["conn_name"]
                    sp = cfg["sp_name"]
                    tipo = cfg["tipo_origen"]  # COLUMNAS o REGISTROS
                    ...
                    # CACHE DEL SP
                    # =============================
                    if key not in cache_sp:

                        params = construir_parametros(
                            cfg, mes_actual, anio_actual, fecha_actual
                        )

                        if tipo == "COLUMNAS":
                            cache_sp[key] = obtener_resultado_columnas(conn, sp, params)
                        else:
                            cache_sp[key] = ejecutar_kpi_registros(
                                conn_name=conn, sp_name=sp, params=params
                            )

                    data_sp = cache_sp[key]
                    ...
                    if tipo == "COLUMNAS":
                        ...
                        resultados_kpi.append(
                            extraer_kpi_de_objeto(data_sp, cfg, anio_actual, mes_actual)
                        )

                    else:
                        resultados_kpi += data_sp

                except Exception as e:
                    # 🔥 NO PARAR TODO
                    resultados_kpi.append(
                        {
                            "codigo_kpi": cfg.get("codigo_kpi"),
                            "anio": anio_actual,
                            "mes": mes_actual,
                            "resultado": 0,
                            "comentario": f"ERROR KPI: {str(e)}",
                        }
                    )
            ...
            # 4) Guardar
            # ================================
            guardados = guardar_kpis_destino(resultados_kpi)

            return JsonResponse(
                {
                    "save": 1,
                    "guardados": guardados,
                    "total_procesados": len(resultados_kpi),
                }
            )
...
class EnviarVouchersPlanilla(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            empleados_seleccionados = request.POST.getlist("empleados[]")
            empleados_seleccionados = set(
                int(e) for e in empleados_seleccionados if str(e).isdigit()
            )

            print("Seleccionados:", empleados_seleccionados)
            ...
            if not encabezado:
                return JsonResponse(
                    {"success": False, "error": "No se encontró encabezado"}
                )

            empresa_nombre = encabezado[0]
            ...
            with connections["universal"].cursor() as cursor:
                ...
                empleados_rows = self.dictfetchall(cursor)

            # 🔥 FILTRAR DESDE AQUÍ (más limpio que usar continue)
            if empleados_seleccionados:
                empleados_rows = [
                    e
                    for e in empleados_rows
                    if e.get("id_empleado") in empleados_seleccionados
                ]

            if not empleados_rows:
                return JsonResponse(
                    {"success": False, "error": "No hay empleados para enviar"}
                )
            ...
            with connections["universal"].cursor() as cursor:
                ...
                detalles_rows = self.dictfetchall(cursor)
            ...
            for d in detalles_rows:
                emp_id = d.get("id_empleado")
                detalles_por_empleado.setdefault(emp_id, []).append(d)
            ...
            for emp in empleados_rows:
                ...
                telegram_id = emp.get("id_telegram")
                correo = emp.get("correo_empresarial")

                salario = float(emp.get("salario_calculado", 0))
                total_beneficios = salario
                total_deducciones = 0

                mensaje = (
                    f"<b>📄 Comprobante de Pago</b>\n\n"
                    f"<b>{empresa_nombre}</b>\n"
                    f"<b>Empleado:</b> {emp.get('nombre_completo')}\n"
                    f"<b>Código:</b> {emp.get('codigo_empleado')}\n"
                    f"<b>Planilla:</b> {emp.get('tipo_planilla')}\n"
                    f"<b>Fecha:</b> {emp.get('fecha_planilla')}\n"
                    f"\n━━━━━━━━\n"
                    f"<b>💰 BENEFICIOS</b>\n"
                    f"• Salario: L {salario:,.2f}\n"
                )

                detalles_emp = detalles_por_empleado.get(emp_id, [])

                for d in detalles_emp:
                    tipo = d.get("tipo_detalle")
                    monto = float(d.get("monto", 0))

                    if tipo == "beneficio":
                        mensaje += f"- {d.get('descripcion_tipo')}: L {monto:,.2f}\n"
                        total_beneficios += monto
                    elif tipo == "deduccion":
                        total_deducciones += monto

                mensaje += f"\nTotal Beneficios: L {total_beneficios:,.2f}"
                mensaje += "\n━━━━━━━━\n"
                mensaje += "<b>📉 DEDUCCIONES</b>\n"

                for d in detalles_emp:
                    if d.get("tipo_detalle") == "deduccion":
                        monto = float(d.get("monto", 0))
                        mensaje += f"- {d.get('descripcion_tipo')}: L {monto:,.2f}\n"

                total_neto = total_beneficios - total_deducciones

                mensaje += f"\nTotal Deducciones: L {total_deducciones:,.2f}"
                mensaje += "\n━━━━━━━━\n"
                mensaje += f"<b>💵 NETO A PAGAR: L {total_neto:,.2f}</b>"
            ...
            return JsonResponse(
                {
                    "success": True,
                    "telegram_enviados": enviados_telegram,
                    "correo_enviados": enviados_correo,
                }
            )
...
class ObtenerEmpleadoPorCodigo(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # ============================
        # OBTENER DATA (JSON o FORM)
        # ============================
        if request.content_type == "application/json":
            data = json.loads(request.body.decode("utf-8"))
        else:
            data = {"codigo_empleado": request.POST.get("codigo_empleado", "")}

        codigo_empleado = data.get("codigo_empleado")

        if not codigo_empleado:
            return JsonResponse(
                {"success": False, "error": "Debe enviar codigo_empleado"}
            )
        ...
        try:
            ...
            # ============================
            with connections["universal"].cursor() as cursor:
                cursor.callproc("TH_GET_DATA_X_EMPLEADO_X_CODIGO", [codigo_empleado])
            ...
            return JsonResponse({"success": True, "data": empleado})
