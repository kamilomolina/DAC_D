...
def nueva_asistencia(request):
    ...
    context = {
        ...
        'deptosData':obtener_departamentos(),
        'varAcceso':varAcceso,
    }
...
def reporte_semanal_asistencias(request):
    user_id = request.session.get('user_id', '')  
...
def incidencias(request):
    id = request.session.get('user_id', '')
    configIncidencias = request.session.get('configIncidencias', 0)
    talentoHumanoAdminIT = request.session.get('talentoHumanoAdminIT', 0)

    if id == '':
        return HttpResponseRedirect(reverse('login'))
    else:
        context = {
            "configIncidencias": configIncidencias,
            "talentoHumanoAdminIT": talentoHumanoAdminIT,
        }

        return render(request, 'asistencia/incidencias.html', context)
    ...
    if request.method == 'GET':
        ...
        try:
            with connections['universal'].cursor() as cursor:
                # Llamar al procedimiento almacenado
                cursor.callproc('TH_GET_DEPTOS_X_USER', [user_id, varAcceso])
                resultados = cursor.fetchall()
...
def mostrar_asistencia_marcados(request):
    try:
        ...
        with connections['universal'].cursor() as cursor:
            cursor.callproc("TH_ASISTENCIA_VISUALIZAR_MARCACIONES_v2", [fecha_inicio, fecha_fin, user_id, varAcceso, depto])
            column_names = [desc[0] for desc in cursor.description]
...
def actualizar_incidencia_registrada(request):
    try:
        user_id = request.session.get('user_id', 0)

        id_incidencia = request.POST.get('id_incidencia')
        id_tipo_incidencia = request.POST.get('id_tipo_incidencia')
        descripcion = request.POST.get('descripcion')

        if not id_incidencia or not id_tipo_incidencia or not descripcion:
            return JsonResponse({'save': 0, 'error': 'Datos incompletos'}, status=400)

        with connections['universal'].cursor() as cursor:
            cursor.callproc(
                "TH_UPDATE_INCIDENCIAS_REGISTRADAS",
                [id_incidencia, id_tipo_incidencia, descripcion, user_id]
            )

            # el SP debe retornar: save, error
            columns = [desc[0] for desc in cursor.description]
            row = cursor.fetchone()
            resp = dict(zip(columns, row)) if row else {'save': 0, 'error': 'Sin respuesta del SP'}
...
def reporte_data_asistencia_marcados(request):
    try:
        user_id = request.session.get('user_id', 0)
        ...
        with connections['universal'].cursor() as cursor:
            cursor.callproc("TH_ASISTENCIA_VISUALIZAR_MARCACIONES_v2_X_DATE", [fecha_inicio, fecha_fin, depto, user_id, varAcceso])
            column_names = [desc[0] for desc in cursor.description]
            crudMarcaciones = [dict(zip(column_names, row)) for row in cursor.fetchall()]
        return JsonResponse({'data': crudMarcaciones}, safe=False)
...
def obtener_departamentos():
    with connections['universal'].cursor() as cursor:
        cursor.callproc('TH_GET_DEPARTAMENTOS')  
        column_names = [desc[0] for desc in cursor.description]
        results = [dict(zip(map(str, column_names), row)) for row in cursor.fetchall()]
