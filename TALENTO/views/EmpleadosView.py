    ...
    with appConn.cursor() as cursor:
        ...
        if menuQuery:
            for menu in menuQuery:
                ...
                if posicion_menu == 'TH204' and permiso_menu == 1:
                    request.session['configIncidencias'] = 1
                elif posicion_menu == 'TH204' and permiso_menu == 0:
                    request.session['configIncidencias'] = 0
        appConn.close()
...
def gestion_empleado(request, parametro=None):
    ...
    if id == '':
        ...
        empleados = get_empleados_all()

        context = {
            'parametro': parametro,
            'id_empleado': parametro,
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            with connections['global_local'].cursor() as cursor_local:
                cursor_local.execute('CALL TH_INSERT_GRUPOS(%s, %s, %s, %s)', [
                    nombre, 
                    descripcion, 
                    11,  # fkModulo predeterminado a 11
                    0    # isBuiltIn predeterminado a 0 (falso)
                ])
                resultado_local = cursor_local.fetchone()
                nuevo_grupo_id = resultado_local[0] if resultado_local else None

            if nuevo_grupo_id:
                try:
                    # Intentar insertar en la base de datos de la nube
                    with connections['global_nube'].cursor() as cursor_nube:
                        cursor_nube.execute('CALL TH_INSERT_GRUPOS(%s, %s, %s, %s)', [
                            nombre, 
                            descripcion, 
                            11,  # fkModulo predeterminado a 11
                            0    # isBuiltIn predeterminado a 0 (falso)
                    ...
                    # Si todo sale bien en la nube, devolver éxito
                    return JsonResponse({'status': 'success', 'id': nuevo_grupo_id, 'message': 'Departamento creado exitosamente'})

                except OperationalError as e_nube:
                    # Si falla en la nube, hacer rollback en la base de datos local
                    with connections['global_local'].cursor() as cursor_local:
                        cursor_local.execute('DELETE FROM grupos WHERE id = %s', [nuevo_grupo_id])

                    return JsonResponse({'status': 'fail', 'error': 'Error al insertar en la nube. Rollback realizado en local.'}, status=500)

            else:
                return JsonResponse({'status': 'fail', 'error': 'No se pudo obtener el ID del grupo recién insertado.'}, status=500)

        except OperationalError as e_local:
            # Manejar errores en la base de datos local
            error_message = str(e_local)
            logger.error(f"Error en la base de datos local: {error_message}")
            if "El nombre del departamento ya existe" in error_message:
                ...
                return JsonResponse({'status': 'fail', 'error': 'Error en la base de datos local.'}, status=500)
        except Exception as e:
...
def crear_area(request):
    if request.method == 'POST':
        ...
        try:
            # Paso 1: Insertar en la base de datos local y obtener el ID insertado
            with connections['global_local'].cursor() as cursor_local:
                cursor_local.execute('CALL TH_INSERT_SUB_GRUPOS(%s, %s, %s)', [
                    sub_grupo, 
                    FKgrupo, 
                    creado_por
                ])
                resultado_local = cursor_local.fetchone()
                nuevo_sub_grupo_id = resultado_local[0] if resultado_local else None

            if nuevo_sub_grupo_id:
                try:
                    # Paso 2: Insertar en la base de datos de la nube
                    with connections['global_nube'].cursor() as cursor_nube:
                        cursor_nube.execute('CALL TH_INSERT_SUB_GRUPOS(%s, %s, %s)', [
                            sub_grupo, 
                            FKgrupo, 
                            creado_por
                        ])

                    # Si ambas inserciones son exitosas, retornar éxito
                    return JsonResponse({'status': 'success', 'id': nuevo_sub_grupo_id, 'message': 'Área creada exitosamente.'})

                except Exception as e_nube:
                    # Si la inserción en la nube falla, hacer rollback en la base de datos local
                    with connections['global_local'].cursor() as cursor_local:
                        cursor_local.execute('DELETE FROM sub_grupos WHERE id = %s', [nuevo_sub_grupo_id])

                    logger.error(f"Error en la nube, rollback en local: {str(e_nube)}")
                    return JsonResponse({'status': 'fail', 'error': 'Error al insertar en la nube. Rollback realizado en local.'}, status=500)
            ...
            logger.error(f"Error inesperado: {str(e_local)}")
            return JsonResponse({'status': 'fail', 'error': f'Ocurrió un error inesperado: {str(e_local)}'}, status=500)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            with connections['global_local'].cursor() as cursor_local:
                ...
                resultado_local = cursor_local.fetchone()
                if resultado_local:
                    nuevo_cargo_id = resultado_local[0]  # Obtener el ID del cargo
                else:
                    return JsonResponse({'status': 'fail', 'error': 'No se pudo obtener el ID del cargo recién insertado.'}, status=500)

            # Conexión a la base de datos 'global_nube' para sincronizar el cargo
            with connections['global_nube'].cursor() as cursor_nube:
                cursor_nube.callproc('TH_INSERT_CARGOS_AREA', [
                    nombre,
                    grado,
                    id_area,
                    creado_por,
...
def contar_empleados(request):
    with connections['talento'].cursor() as cursor:
...
def obtener_empresas():
    ...
    try:
        ...
        print(e)
    return empresas_list  # Devolver la lista de empresas en lugar de un JsonResponse
...
def get_empleados_all():
    with connections['talento'].cursor() as cursor:
        cursor.callproc('TH_GET_EMPLEADOS_ALL')
        resultados = cursor.fetchall()     
    return resultados
...
def desactivar_familiar(request):
    if request.method == 'POST':
        ...
        if not id_familiar:
            return JsonResponse({'status': 'fail', 'error': 'ID del familiar es requerido.'}, status=400)

        try:
            with connections['talento'].cursor() as cursor:
                # Llamar al procedimiento almacenado para desactivar el familiar
                cursor.callproc('TH_DELETE_EMPLEADOS_FAMILIARES', [id_familiar, modificado_por])
            
            return JsonResponse({'status': 'success', 'message': 'Familiar desactivado exitosamente.'})
...
def crear_rol_empleado(request):
    if request.method == 'POST':
        ...
        try:
            with connections['talento'].cursor() as cursor:  # Reemplaza 'default' por el nombre de tu conexión si es diferente
                # Añadir logs para depurar la llamada al procedimiento almacenado
        ...
        try:
            ...
            with connections['talento'].cursor() as cursor:
                cursor.callproc('TH_UPDATE_TH_EMPLEADOS_FICHAS', [
                	...
                	data.get('aplica_rap', 0),
                	data.get('aplica_ihss', 0),
...
def empleados_telegram_json(request):
    with connections['universal'].cursor() as cursor:
        cursor.callproc("TH_GET_EMPLEADOS_TELEGRAM_LISTADO")
        data = [
...
def empleados_telegram_update(request):
    if request.method == "POST":
        data_json = request.POST.get("data_json")

        with connections['universal'].cursor() as cursor:
            cursor.callproc(
                "TH_UPDATE_EMPLEADOS_TELEGRAM_MASIVO",
                [data_json, request.user.username]
            )
