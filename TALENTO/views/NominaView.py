    ...
    try:
        ...
        with udcConn.cursor() as cursor:
            ...
            if results:
                for result in results:
                    pendientes = result[0]
                    pendientes_text = result[1]


        udcConn.close()

        datos = {'save': 1, 'aplica': aplica, 'pendientes': pendientes, 'pendientes_text': pendientes_text}
    except Exception as e:
...
def validar_aplicar_planilla(request):
    try:
        id_planilla = request.POST.get('id_planilla')
        ...
        with conn.cursor() as cursor:
            cursor.callproc('TH_VALIDAR_APLICAR_PLANILLA', [id_planilla])
            # El SP devuelve 2 resultsets:
            # 1️⃣ Detalle
            listado = cursor.fetchall()

            cursor.nextset()  # ← Mover al siguiente resultset
            conteo = cursor.fetchone()

            if conteo:
                total_inconsistencias = conteo[0]
...
def mostrar_tipos_deducciones(request):
    try:
        ...
        with connections['universal'].cursor() as cursor:
            cursor.callproc("TH_MOSTRAR_TIPOS_DEDUCCIONES")
            column_names = [desc[0] for desc in cursor.description]
            deducciones = [dict(zip(column_names, row)) for row in cursor.fetchall()]
...
def guardar_tipo_deduccion(request):
    if request.method == 'POST':
        try:
            ...
            usa_cuentas_x_centro = data.get('usa_cuentas_x_centro')
...
def obtener_detalles_planilla_v2(request):
    ...
    try:
        udcConn = connections['universal']
        with udcConn.cursor() as cursor:
            cursor.callproc("TH_GET_DETALLES_PLANILLAS_v20251010", [id_planilla, modo_agrupacion])
            column_names = [desc[0] for desc in cursor.description]
...
def obtener_detalles_planilla(request):
    if request.method != 'POST':
        return JsonResponse(
            {'status': 'fail', 'error': 'Método de solicitud no permitido'},
            status=405
        )
    ...
    if not id_planilla:
        return JsonResponse(
            {'status': 'fail', 'error': 'ID de planilla no proporcionado'},
            status=400
        )
    ...
    try:
        with connections['universal'].cursor() as cursor:
            cursor.callproc(
                "TH_GET_DETALLES_PLANILLAS_v20251010",
                [id_planilla, modo_agrupacion]
            )

            colnames = [col[0].lower() for col in cursor.description]
            rows = cursor.fetchall()

        detalles = [dict(zip(colnames, row)) for row in rows]

        return JsonResponse({
            'status': 'success',
            'data': detalles  # <-- AQUI EL CAMBIO IMPORTANTE
        })
...
def mostrar_tipos_bonificaciones(request):
    try:
        with connections['universal'].cursor() as cursor:
            ...
            column_names = [desc[0] for desc in cursor.description]
            bonificaciones = [dict(zip(column_names, row)) for row in cursor.fetchall()]
...
def registrar_saldos_empleados_planillas(request):
    try:
        ...
        with connections['universal'].cursor() as cursor:
            ...
            cursor.callproc('CONTA_GET_VALORES_PLANILLA_PATRONALES', [id_planilla, userName])
...
def config_preaviso_rangos(request):
    ...
    if user_id == '':
        ...
        return render(request, 'planilla/config_preaviso_rangos.html', context)


def nomina_listar_rangos_preaviso(request):
    data = {'success': False, 'data': [], 'error': ''}

    try:
        with connections['universal'].cursor() as cursor:
            cursor.callproc('TH_PREAVISO_LISTAR_RANGOS')
            rows = cursor.fetchall()
...
def nomina_guardar_rango_preaviso(request):
    data = {'save': 0, 'error': ''}

    try:
        opc = int(request.POST.get('opc', 1))
        id_rango = request.POST.get('id_rango') or None
        meses_min = int(request.POST.get('meses_min', 0))

        meses_max_raw = request.POST.get('meses_max', '').strip()
        meses_max = int(meses_max_raw) if meses_max_raw != '' else None
        ...
        with connections['universal'].cursor() as cursor:
            cursor.callproc(
                'TH_PREAVISO_GUARDAR_RANGO',
                [opc, _id, meses_min, meses_max, dias, userName]
            )
            res = cursor.fetchall()
            if res:
                row = res[0]
...
def nomina_cambiar_estado_rango_preaviso(request):
    data = {'save': 0, 'error': ''}

    try:
        id_rango = int(request.POST.get('id_rango'))
        userName = request.session.get('userName', 'SYSTEM')

        with connections['universal'].cursor() as cursor:
            cursor.callproc('TH_PREAVISO_CAMBIAR_ESTADO', [id_rango, userName])
            res = cursor.fetchall()
            if res:
                data['save'] = int(res[0][0])
            else:
                data['save'] = 0
    except Exception as e:
        data['error'] = str(e)
    ...
    if user_id == '':
        ...
        try:
            ...
            with udcConn.cursor() as cursor:
                ...
                column_names = [desc[0] for desc in cursor.description]
                partidasAutomaticas = [dict(zip(map(str, column_names), row)) for row in cursor.fetchall()]
            udcConn.close()
        except Exception as e:
            partidasAutomaticas = str(e)
        ...
        return render(request, 'planilla/tipos_planillas.html', context)



def nomina_listar_clases_planilla(request):
    try:
        with connections["universal"].cursor() as cursor:
            cursor.callproc("TH_CLASES_PLANILLA_LISTAR")
            column_names = [desc[0] for desc in cursor.description]
...
def nomina_guardar_clase_planilla(request):
    try:
        opc = request.POST.get('opc')
        id_clase_planilla = request.POST.get('id_clase_planilla')
        descripcion = request.POST.get('descripcion_cargo_laboral')
        tipo = request.POST.get('tipo')
        valor = request.POST.get('valor')
        codigo_partida = request.POST.get('codigo_partida_automatica')
        usa_cuenta = request.POST.get('usa_cuenta_por_empleado')
        codigo_cuenta = request.POST.get('codigo_cuenta')
        registrar_gasto = request.POST.get('registrar_gasto')
        aplica_patronal = request.POST.get('aplica_patronal')

        userName = request.session.get('userName', '')

        appConn = connections["universal"]
        with appConn.cursor() as cursor:
            cursor.callproc('TH_CLASES_PLANILLA_GUARDAR', [
                opc,
                id_clase_planilla,
                descripcion,
                tipo,
        ...
        return JsonResponse({'save': 1, 'result': result})
...
def nomina_cambiar_estado_clase_planilla(request):
    try:
        id_clase_planilla = request.POST.get('id_clase_planilla')
        userName = request.session.get('userName', '')
        ...
        appConn = connections["universal"]
        with appConn.cursor() as cursor:
            cursor.callproc('TH_CLASES_PLANILLA_CAMBIAR_ESTADO', [
                id_clase_planilla,
                userName
            ])
            result = cursor.fetchall()
...
# Obtener detalle contable de planilla
    
def obtener_detalle_contable_planilla(request):
    if request.method != 'POST':
        return JsonResponse(
            {'status': 'fail', 'error': 'Método de solicitud no permitido'},
            status=405
    ...
    # userName desde sesión (como manejás todo el sistema)
    userName = request.session.get('userName')

    if not userName:
        return JsonResponse(
            {'status': 'fail', 'error': 'Sesión inválida'},
            status=401
        )
    ...
    try:
        with connections['universal'].cursor() as cursor:
            cursor.callproc(
                'CONTA_GET_VALORES_PLANILLA_PATRONALES_DETAILS',
                [int(id_planilla), userName]
            )
